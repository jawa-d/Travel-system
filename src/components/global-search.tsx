        ...
        <div role="dialog" aria-modal="true" aria-label="البحث الشامل" className="fixed inset-0 z-[70] bg-slate-950/55 p-4 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}>
          <div className="mx-auto mt-[10vh] max-w-2xl overflow-hidden rounded-2xl border bg-card text-card-foreground shadow-2xl">
            <div className="flex items-center gap-2 border-b p-3">
              ...
              <Input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ابحث عن عميل، جواز، وثيقة أو وجهة..." className="border-0 shadow-none focus-visible:ring-0" />
            ...
            <div className="max-h-[55vh] overflow-y-auto p-2">
              ...
              {query.length < 2 ? <p className="p-8 text-center text-sm text-muted-foreground">اكتب حرفين على الأقل للبحث في بيانات الخادم.</p> : null}
            </div>
          </div>
        </div>
      ) : null}
