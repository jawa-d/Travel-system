...
export function AppNavigation({ locale, role, mobileOnly = false }: { locale: Locale; role: Role; mobileOnly?: boolean }) {
    ...
    return (
      <Link
        ...
        className={cn(
          ...
          selected && "bg-primary text-white shadow-md shadow-primary/15",
          !selected && item.featured && "bg-primary/10 text-primary hover:bg-primary/15",
          !selected && !item.featured && "text-slate-600 hover:bg-white hover:text-slate-950 hover:shadow-sm"
        )}
      >
        <span className={cn(
          "grid h-8 w-8 shrink-0 place-items-center rounded-lg transition-colors",
          selected ? "bg-white/15" : item.featured ? "bg-primary/10" : "bg-slate-100 group-hover:bg-primary/10 group-hover:text-primary"
        )}>
  ...
  const sidebar = (
    <aside className={cn(
      "fixed right-0 top-0 z-50 flex h-full w-[292px] flex-col border-l border-slate-200/80 bg-[#f8fafc] transition-transform duration-300 dark:border-border dark:bg-card",
      ...
      <div className="flex h-[73px] shrink-0 items-center justify-between border-b border-slate-200/70 px-5">
        ...
        {mobileOnly ? (
          <button type="button" onClick={() => setOpen(false)} className="grid h-9 w-9 place-items-center rounded-lg text-slate-500 hover:bg-white" aria-label="إغلاق">
            <X className="h-5 w-5" />
          </button>
        ) : null}
      </div>
      ...
      <nav className="flex-1 space-y-5 overflow-y-auto px-4 py-5">
        {visibleGroups.map((group) => group.collapsible ? (
          <details key={group.en} className="group/settings" open={group.items.some((item) => active(item.href)) || undefined}>
            <summary className="mb-1 flex cursor-pointer list-none items-center justify-between rounded-lg px-2 py-1.5 text-[10px] font-extrabold uppercase tracking-[0.12em] text-slate-400 hover:text-slate-600">
      ...
      <div className="shrink-0 border-t border-slate-200/70 p-4">
        <div className="rounded-xl border border-primary/10 bg-gradient-to-l from-primary/[0.08] to-cyan-500/[0.04] p-3">
          ...
          <p className="mt-1 text-[10px] leading-4 text-slate-500">{isArabic ? "يتم حفظ تغييراتك تلقائياً." : "Your changes are saved automatically."}</p>
        </div>
      </div>
    </aside>
  );
  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-slate-700 shadow-sm lg:hidden" aria-label="القائمة">
