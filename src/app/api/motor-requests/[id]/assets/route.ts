...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const pathname = `motor-request-assets/${id}/${kind}/${Date.now()}-${crypto.randomUUID()}-${safeFileName(file.name)}`;
    ...
    const updated = await prisma.motorInsuranceRequest.update({ where: { id }, data: { [kind]: asset } });
    await writeAuditLog({
      userId: user.id,
      role: user.role,
      action: "MOTOR_REQUEST_APPROVAL_ASSET_UPLOADED",
